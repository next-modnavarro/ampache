...
if [ "$1" == "backup" ]; then
    ...
    for db in $databases; do
        ...
        mysqldump --force --opt --host=$MYSQL_HOST --port=$MYSQL_PORT --user=$MYSQL_USER --databases $db ${PASS_OPT} | gzip > "/tmp/$db.sql.gz"

        if [ $? == 0 ]; then
            aws s3 cp /tmp/$db.sql.gz s3://$S3_BUCKET/$S3_PATH/$db.sql.gz

            if [ $? == 0 ]; then
                rm /tmp/$db.sql.gz
            else
                >&2 echo "couldn't transfer $db.sql.gz to S3"
            fi
        else
